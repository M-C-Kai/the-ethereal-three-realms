...
- 当前 Python 本地服文档覆盖 33 个唯一 ID；APK 另外保留 **87 个 ID**。
- 下表中 **19 个**额外 ID 只有 APK 接收分支，当前仓库既没有 APK 发送调用，也没有本地服发送实现，因此按用户要求标记为“未使用/无发送调用证据”。这不代表旧官方服务端一定从未发送。
...
| 1056 | 队伍界面；`pmsj/work/e/er.smali:1441`、`pmsj/work/e/et.smali:935` | 组队、邀请、申请；已确认模块 |
...
| 1023 | 双向 | 接收 main/e.ag，分发:8421<br>发送 pmsj/work/b/aa.smali:429（42处） | APK静态使用；本地服未接入 |
...
| 1026 | S→C | 接收 main/e.aj，分发:8426 | 未使用/无发送调用证据 |
...
以下 19 个 ID 只有 `main/e` 的服务端消息接收分支，未找到 APK 发送构造，当前 Python 本地服也没有对应发送函数：

`1003, 1014, 1015, 1026, 1038, 1066, 1078, 1096, 1115, 1141, 1142, 1144, 1170, 1505, 1512, 2028, 2030, 2100, 5020`
