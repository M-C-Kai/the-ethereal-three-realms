...
from systems.skill.registry import (
...
class SkillService:
    ...
    def __init__(self, life_registry, item_registry, save):
        self.life_registry = life_registry
        self.item_registry = item_registry
        self.save = save

    def craft(self, role, fields, protocol):
        if not (protocol.is_life_craft_request(fields) or protocol.is_life_direct_use_request(fields)):
            return LifeTransactionResult((), False, '制造请求格式非法')
        recipe_id = int(fields[1].value)
        if protocol.is_life_direct_use_request(fields):
            slots, quantity = [0, 0, 0, 0], 1
        else:
            slots = [int(field.value) for field in fields[2:6]]
            quantity = int(fields[6].value)
        snapshot = copy.deepcopy(role)
        try:
            result = craft_result(role, self.life_registry.recipe(recipe_id), slots, quantity, life_registry=self.life_registry, item_registry=self.item_registry)
            if result.changed:
                self.save()
            return result
        except Exception:
            role.clear(); role.update(snapshot); raise

    def learn(self, role, fields, trainer_id=0):
        entry = self.life_registry.learnable.get(int(fields[1].value))
        trainer = self.life_registry.trainers.get(int(trainer_id))
        snapshot = copy.deepcopy(role)
        try:
            result = learn_result(role, trainer, entry, life_registry=self.life_registry)
            if result.changed:
                self.save()
            return result
        except Exception:
    ...
    def upgrade(self, role, fields):
        skill = self.life_registry.skills.get(int(fields[1].value))
        snapshot = copy.deepcopy(role)
        try:
            result = upgrade_result(role, skill, life_registry=self.life_registry)
            if result.changed:
                self.save()
            return result
        except Exception:
    ...
    def complete_gathering(self, role, target):
        snapshot = copy.deepcopy(role)
        try:
            result = gathering_reward_result(role, target, life_registry=self.life_registry, item_registry=self.item_registry)
            if result.changed:
                self.save()
            return result
        except Exception:
