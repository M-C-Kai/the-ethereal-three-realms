...
def default_pet_registry() -> PetRegistry:
    return PetRegistry.from_path(Path(__file__).resolve().parents[2] / 'data' / 'pets.json')
...
def default_pet_skill_registry() -> PetSkillRegistry:
    return PetSkillRegistry.from_path(Path(__file__).resolve().parents[2] / 'data' / 'pet_skills.json')
