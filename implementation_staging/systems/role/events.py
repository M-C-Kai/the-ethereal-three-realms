"""角色系统事件与角色刷新总线（自 character_update_bus.py 收编）。"""
...
class CharacterUpdateBus:
    ...
    def __init__(
        ...
        on_publish: Callable[[dict[str, object]], None] | None = None,
    ) -> None:
        ...
        # 可选的发布后回调（携带已变更的 role dict）。用于把外观变化同步给
        # 视野内的其他在线客户端（同图互见的 1017 定向刷新）。
    ...
    def _notify(self, result: CharacterUpdateResult, role: dict[str, object]) -> None:
        """Fire the post-publish callback only when frames were produced."""
        if self._on_publish is not None and result.frames:
            self._on_publish(role)
