...
`登录 -> 本地一区 -> 角色列表 -> 创建/删除角色 -> 进入地图 -> 1126 试炼妖兽实体 -> 背包 -> 人物四页面板 -> 装备/卸下/使用物品 -> 长安传送阵 -> 昆仑导师 -> 门派技能学习`
...
- 本地地图 `60001`“昆仑”使用独立 32×32 功能地图；长安 `580005` 与昆仑 `6000101` 构成双向传送。昆仑导师 `1900101` 通过原生 `2032` option 1 打开 `1010/action=69` 的门派技能 mode 1，实际学习继续使用并持久化 `1103/action=3`。该地图 ID 和布局不代表原服还原。
...
7. `58.map.o` 在本地注册表中命名为“长安”；50000 使用本地生成的独立测试资源，60001 是本地昆仑功能地图。三者都不是对原服完整地图的还原。
