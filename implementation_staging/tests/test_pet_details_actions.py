...
class PetDetailsActionsTests(unittest.TestCase):
    ...
    def test_1127_action_9_maps_properties_30_through_90(self):
        message_id, fields = decode_frame(pet_detail_frame(self.pet, self.registry))
        ...
        self.assertEqual(self.property_field(fields, 38).value, self.property_field(fields, 39).value)
    ...
    def test_1103_action_10_empty_pet_skill_container(self):
        ...
        self.assertEqual(
            ...
            [(TYPE_BYTE, 10), (TYPE_INT, self.pet['id']), (TYPE_BYTE, 0), (TYPE_BYTE, 0)],
        )

    def test_1134_generic_property_update_layout(self):
        message_id, fields = decode_frame(pet_property_update_frame(self.pet['id'], [(11, 1)]))
        self.assertEqual(message_id, 1134)
        self.assertEqual(
            [(field.type_id, field.value) for field in fields],
            [(TYPE_BYTE, 0), (TYPE_INT, self.pet['id']), (TYPE_INT, 1), (TYPE_BYTE, 11), (TYPE_INT, 1)],
        )

    def test_state_transition_reports_property_11_and_12_updates(self):
        result = apply_pet_state_request(self.role, [byte(10), integer(self.pet['id']), byte(1)])
        self.assertEqual(result.updates, ((self.pet['id'], 11, 1),))
        result = apply_pet_state_request(self.role, [byte(48), integer(self.pet['id']), byte(1)])
